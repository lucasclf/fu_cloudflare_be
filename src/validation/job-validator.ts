...
import { CreateJobAliasInput, CreateJobInput, CreateJobQuestionInput } from "../domain/jobs/job";
import { ensureObject, readBooleanWithDefault, readNumberWithDefault, readOptionalString, readRequiredNumber, readRequiredString } from "./generic-validator";
...
export function validateCreateJobQuestionsInput(input: unknown): CreateJobQuestionInput {
  const raw = ensureObject(input);
...
export function validateCreateJobAliasesInput(input: unknown): CreateJobAliasInput { 
  const raw = ensureObject(input);
